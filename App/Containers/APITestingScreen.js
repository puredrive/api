...
const endpoints = [
  ...
  { label: '', endpoint: 'getMovie', args: ['Split','JSON'] }
]
...
export default class APITestingScreen extends React.Component {
  ...
  showResult(response: Object, title: string = 'Response') {
    ...
    if (response.ok) {
      this.refs.testme.setState({ message: FJSON.plain(response.data), title: title })
    } else {
      this.refs.testme.setState({ message: `${response.problem} - ${response.status}`, title: title })
    }
  }
